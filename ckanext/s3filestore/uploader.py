...
class BaseS3Uploader(object):
    ...
    def get_s3_bucket(self, bucket_name=None):
        ...
        try:
            ...
            if error_code == '404':
                log.warning('Bucket %s could not be found, '
                            'attempting to create it...', bucket_name)
                try:
                    bucket = \
    ...
    def upload_to_key(self, filepath, upload_file, make_public=False):
        ...
        try:
            ...
            log.error('Something went very very wrong when uploading to [%s]: %s', filepath, str(e))
            raise e
...
class S3Uploader(BaseS3Uploader):
    ...
    def update_data_dict(self, data_dict, url_field, file_field, clear_field):
        '''Manipulate data from the data_dict. This needs to be called
        before it reaches any validators.
...
class S3ResourceUploader(BaseS3Uploader):
    ...
    def get_path(self, id, filename):
        ...
        <ckanext.s3filestore.aws_storage_path>/resources/<resource_id>/<filename>
