...
apt-get install sudo -qqq || echo "sudo is installed"

#software-properties-common not installed on slim
sudo apt-get install software-properties-common wget curl -y -qqq
...
sudo apt-get install adoptopenjdk-8-hotspot -y -qq
...
sudo dpkg -i adopt-openjdk-8-jdk_0.1_all.deb
